...
fn compute_round (
	...
	let points: Vec<(Vector, u64)> = point_quadtree.flatten();
	...
	let mut new_points: QuadTree = compute_intersections(point_quadtree, lines, &mut new_lines, boundary);
...
fn main () {
	...
	for _i in 0..2 {
		compute_round(&mut points, &mut lines, &unit_square);
